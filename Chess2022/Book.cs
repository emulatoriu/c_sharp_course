...
namespace SrcChess2 {
    ...
    public class Book {
        ...
        /// </summary>
        /// <param name="asm">      Assembly or null for the current one</param>
        ...
        public bool ReadBookFromResource(Assembly? asm, string resName) {
            ...
            stream  = (asm ?? GetType().Assembly).GetManifestResourceStream(resName) ?? throw new InvalidOperationException("Unable to find the opening book in resources");
            using(stream) {
        ...
        /// </summary>
        /// <param name="resName">  Resource Name</param>
        public bool ReadBookFromResource(string resName) => ReadBookFromResource(asm: null, resName);
        ...
        public int CreateBookList(List<short[]> MovesList, int minMoveCount, int maxDepth, PgnParser.delProgressCallBack? callback, object? cookie) {
            ...
            foreach (short[] pnMoveList in MovesList) {
                if (pnMoveList.Length >= minMoveCount) {
                    if (pnLast == null || !CompareList(pnMoveList, pnLast, maxDepth)) {
                        uniqueMovesList.Add(pnMoveList);
                        pnLast = pnMoveList;
                    }
                }
            }
