...
namespace SrcChess2.FICSInterface {
    ...
    public class FICSGame {
        ...
        /// <returns></returns>
        private static string CnvPlayerName(string playerName) {
            if (playerName.Length < 10) {
                return(playerName.PadRight(10));
            } else {
        ...
        private static string TimeToString(TimeSpan span) {
            ...
            if (span.Hours != 0) {
                retVal = span.Hours.ToString(CultureInfo.InvariantCulture) + ":" + span.Minutes.ToString(CultureInfo.InvariantCulture).PadLeft(2, '0') + ":" + span.Seconds.ToString(CultureInfo.InvariantCulture).PadLeft(2, '0');
            } else {
                retVal = span.Minutes.ToString(CultureInfo.InvariantCulture) + ":" + span.Seconds.ToString(CultureInfo.InvariantCulture).PadLeft(2, '0');
            }
            return(retVal);
        }
        ...
        public static char GameTypeToChar(FicsGameType gameType) {
            ...
            switch (gameType) {
            case FicsGameType.Blitz:
                retVal = 'b';
                break;
            case FicsGameType.Examined:
                retVal = 'e';
                break;
            case FicsGameType.Lightning:
                retVal = 'l';
                break;
            case FicsGameType.NonStandard:
                retVal = 'n';
                break;
            case FicsGameType.Standard:
                retVal = 's';
                break;
            case FicsGameType.Untimed:
                retVal = 'u';
                break;
            case FicsGameType.Wild:
                retVal = 'w';
                break;
            case FicsGameType.Atomic:
                retVal = 'x';
                break;
            case FicsGameType.Crazyhouse:
                retVal = 'z';
                break;
            case FicsGameType.Bughouse:
                retVal = 'B';
                break;
            case FicsGameType.Losers:
                retVal = 'L';
                break;
            case FicsGameType.Suicide:
                retVal = 'S';
                break;
            default:
                retVal = 'n';
        ...
        public static TimeSpan ParseTime(string str) {
            ...
            if (arr.Length == 3) {
                arrMS = arr[2].Split('.');
                if (arrMS.Length == 1) {
                    span = new TimeSpan(Int32.Parse(arr[0], CultureInfo.InvariantCulture),
                                        Int32.Parse(arr[1], CultureInfo.InvariantCulture),
                                        Int32.Parse(arr[2], CultureInfo.InvariantCulture));
                } else {
                    span = new TimeSpan(0,
                                        Int32.Parse(arr[0],   CultureInfo.InvariantCulture),
                                        Int32.Parse(arr[1],   CultureInfo.InvariantCulture),
                                        Int32.Parse(arrMS[0], CultureInfo.InvariantCulture),
                                        Int32.Parse(arrMS[1], CultureInfo.InvariantCulture));
                }
            } else {
                arrMS = arr[1].Split('.');
                if (arrMS.Length == 1) {
        ...
        public static FICSGame? ParseGameLine(string str, out bool isSupported) {
            ...
            if (String.IsNullOrWhiteSpace(str)) {
                ...
                try {
                    ...
                    if (!Int32.TryParse(tok, out int gameId)) {
                        retVal  = null;
                    } else {
                        retVal = new FICSGame {
                            GameId = gameId
                        ...
                        if (isExam || tok == "games") {
                            ...
                            if (String.IsNullOrEmpty(enclosedStr)) {
                                ...
                                if (chr != ' ' && chr != 'p') {
                                    ...
                                    retVal.PlayerTimeInMin  = Int32.Parse(GetNextToken(enclosedStr, ref enclosedPos));
                                    retVal.IncTimeInSec     = Int32.Parse(GetNextToken(enclosedStr, ref enclosedPos));
                                    ...
                                    if (String.IsNullOrEmpty(enclosedStr)) {
                                        ...
                                        enclosedPos                 = 0;
                                        retVal.WhiteMaterialPoint   = Int32.Parse(GetNextDigitToken(enclosedStr, ref enclosedPos));
                                        GetNextNonWhiteChar(enclosedStr, ref enclosedPos);
                                        retVal.BlackMaterialPoint   = Int32.Parse(GetNextDigitToken(enclosedStr, ref enclosedPos));
                                        retVal.NextMovePlayer       = ParsePlayer(GetNextToken(str, ref pos)[0]);
                                        retVal.NextMoveCount        = Int32.Parse(GetNextToken(str, ref pos));
                                    }
                                }
                            }
                        }
        ...
        public static bool? ParseMoveLine(int           moveIndex,
            ...
            try {
                if (line.Trim().StartsWith("{")) {
                    ...
                    if (Int32.TryParse(tok, out int intVal) && intVal == moveIndex && GetNextChar(line, ref posInLine) == '.') {
