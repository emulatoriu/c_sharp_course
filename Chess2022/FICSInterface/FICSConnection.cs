...
namespace SrcChess2.FICSInterface {
    ...
    public class FICSConnection : IDisposable {
        ...
        private class AutomatonState : IDisposable {
            ...
            private readonly Dictionary<int,GameIntf>   m_gameIntfDict;
            ...
            public AutomatonState() {
                ...
                m_gameIntfDict      = new Dictionary<int, GameIntf>(16);
            ...
            public void TerminateGame(GameIntf gameIntf, TerminationCode terminationCode, string terminationComment, string? errTxt) {
                lock(m_gameIntfDict) {
            ...
            /// </returns>
            public GameIntf? FindGameIntf(int gameId) {
                GameIntf?   retVal;
            ...
            /// Game or null if not found
            /// </returns>
            public GameIntf? FindGameIntf(ChessBoardControl chessBoardControl) {
                GameIntf?    retVal = null;
            ...
            public void SetCommand(CmdExecuting cmd, GameIntf? gameIntf) {
        ...
        public bool DebugTrace {
            get => (m_connection != null) ? m_connection.DebugTrace : false;
            set {
        ...
        private void SetSetting(string settingName, string value, bool addToSet) {
            if (!addToSet || m_variableDict.ContainsKey(settingName)) {
                m_connection!.SendLine(String.Format("set {0} {1}", settingName, value));
                if (addToSet &&
        ...
        private void SetISetting(string settingName, string value) {
            m_connection!.SendLine(String.Format("iset {0} {1}", settingName, value));
        }
        ...
        public void RestoreOldSetting() {
            ...
            foreach (string strSetting in m_changedSettingsSet) {
                ...
                SetSetting(strSetting, oldValue, false /*bAddToSet*/);
            }
            m_connection.FlushInput();
        }
        ...
        public bool Login(string        password,
            ...
            m_state!.SetCommand(CmdExecuting.Login, null /*gameIntf*/);
        ...
        /// true if succeed, false if game is already defined
        /// </returns>
        public bool ObserveGame(GameIntf gameIntf, int timeOut, out string? errTxt) {
            bool    retVal;
            int     gameId;
        ...
        /// Timeout action
        /// </returns>
        public Action<GameIntf> GetTimeOutAction() => (x) => { m_state!.TerminateGame(x, TerminationCode.TerminatedWithErr, "", "Move timeout"); };
        ...
        /// true if succeed, false if game is already defined
        /// </returns>
        public bool ObserveGame(FICSGame game, ChessBoardControl chessBoardControl, int timeOut, int? moveTimeOut, Action<GameIntf,TerminationCode,string> gameFinishedAction, out string? errTxt) {
            bool                retVal;
            GameIntf            gameIntf;
            Action<GameIntf>?   moveTimeOutAction;

            if (moveTimeOut == 0) {
                moveTimeOutAction = null;
            } else {
                moveTimeOutAction = GetTimeOutAction();
            }
            gameIntf = new GameIntf(game, chessBoardControl, moveTimeOut, moveTimeOutAction, gameFinishedAction);
#if DEBUG
            timeOut    = 3600;
#endif
        ...
        public bool TerminateObservation(ChessBoardControl chessBoardControl) {
            ...
            if (gameIntf == null) {
                ...
                retVal = true;
                m_state.TerminateGame(gameIntf, TerminationCode.TerminatedWithErr, "", "Stop by user");
            }
            return(retVal);
        }
        ...
        public List<string>? GetServerDate(int iTimeOut) {
            ...
            m_state!.SetCommand(CmdExecuting.Date, null /*gameIntf*/);
        ...
        private void ProcessMoveListLine(string line) {
            ...
            if (result == null) {
                ...
                if (!gameIntf.CreateInitialBoard(out errTxt)) {
                    m_state.TerminateGame(gameIntf, TerminationCode.TerminatedWithErr, "", errTxt);
                }
        ...
        private void ProcessLine(string line) {
            ...
            phase                  = m_state.Phase;
            if (!singleMoveListening ||
                phase != 0           ||
                !ProcessSingleMove(line)) {
