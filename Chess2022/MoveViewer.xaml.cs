...

        ...
        private string GetMoveDesc(MoveExt move) {
            ...
            if (m_displayMode == ViewerDisplayMode.MovePos) {
                retVal = ChessBoard.GetHumanPos(move);
            } else {
                retVal = PgnUtil.GetPGNMoveFromMove(m_chessCtl!.Board, move, false);
            ...
            if (chessBoard != null) {
                ...
                if (moveCount != 0) {
                    ...
                    for (int i = 0; i < moveCount; i++) {
                        move = movePosStack[i];
                        if (m_displayMode == ViewerDisplayMode.MovePos) {
                            moveTxt     = ChessBoard.GetHumanPos(move);
        ...
        private void ListViewMoveList_SelectionChanged(object? sender, SelectionChangedEventArgs e) {
            ...
            if (!m_ignoreChg && !m_chessCtl!.IsBusy && !m_chessCtl.IsSearchEngineBusy) {
                m_ignoreChg    = true;
                chessBoard     = m_chessCtl.Board;
                curPos         = chessBoard.MovePosStack.PositionInList;
