class Maerchen
{
    ...
    const string sHaenselUndGraetel = @"Es war einmal ein armer Holzfäller, der mit seinen Kindern, Hänsel und Gretel, am Rande eines großen Waldes lebte. Ihre Mutter war gestorben und der Vater hatte wieder geheiratet.
...
Er war sehr glücklich, dass er seine lieben Kinder wieder hatte und sie lebten in lauter Freude zusammen.";
    ...
    const string sRotkaeppchen = @"Es war einmal ein süßes Mädchen, das mit seiner Mutter auf dem Dorf lebte. Seine Großmutter schenkte ihm ein rotes Käppchen, das ihm so gut stand, dass es nichts anderes mehr tragen wollte. So nannte jeder das Mädchen 'Rotkäppchen'.
...
Auch die Großmutter konnte aus dem Bauch des Wolfes gerettet werden.Der Jäger, Rotkäppchen und die Großmutter holten große Steine und füllten damit den Bauch des Wolfes.Als er aufwachte, wollte er fortspringen, aber die Steine in seinem Leib waren so schwer, dass er gleich niedersank und tot umfiel.Der Jäger zog seinen Pelz ab und ging damit nach Hause.Die Großmutter aß den Kuchen und trank den Wein und erholte sich wieder.Das Rotkäppchen dachte, dass sie nie wieder den Weg verlassen wolle, den sie eigentlich gehen sollte.";
    ...
    const string sAschenputtel = @"Das Märchen beginnt damit, dass die Frau von einem reichen Mann stirbt. Bevor sie die Augen schließt, holt sie ihre einzige Tochter an ihr Krankenbett und sagt ihr: „Liebes Kind, bleib fromm und gut, so wird dir der liebe Gott immer beistehen, und ich will vom Himmel auf dich herabblicken und will um dich sein.
...
kein Blut im Schuck: der Schuck ist nicht zu klein, die rechte Braut, die führt er heim”, gurren die Tauben.Zur Hochzeit wollen sich die Stiefschwestern einschleimen.Doch die Tauben picken beiden jeweils ein Auge aus.Für ihr Bosheit und ihre Falschheit sollen sie ihr Leben lang gestraft sein.";
    ...
    private static int kidWantsSmthProbability = 10;

    private const int WORDS_TILL_ACTION = 100;
    private const int READING_SPEED = 1;
    private const int PAUSE_AFTER_DOT = 1;

    private const double MAX_READING_TIME = 60000;

    private static double currentReadTime = 0;

    /// <summary>
    /// EIne Funktion, die einen Text vorgelesen ausgibt
    /// </summary>
    /// <param name="sGeschichte"></param>
    /// <returns>true, wenn die Geschichte bis zum Ende vorgelesen werden konnte, sonst false</returns>
    public static bool vorlesen(string sGeschichte)
    {        
        for (int i = 0; i < sGeschichte.Length; i++)
        {                        
            if (i % WORDS_TILL_ACTION == 0 && i > 0)
            {
                if (doesKidWantSmth() == true)
                {
                    string[] kidAsksSmth = new string[4];
                    kidAsksSmth[0] = "Papa, ich möchte etwas trinken.";
                    kidAsksSmth[1] = "Papa, ich muss auf Toilette.";
                    kidAsksSmth[2] = "Papa, ich kann nicht schlafen.";
                    kidAsksSmth[3] = "Papa, ich will eine andere Geschichte hören.";

                    int randomProb = new Random().Next(0, kidAsksSmth.Length);
                    switch (randomProb)
                    {
                        case 0:
                        case 1:
                            KidWaitingTime(kidAsksSmth[randomProb]);
                            break;
                        case 2:
                            if (currentReadTime < MAX_READING_TIME)
                            {


                                Color("papa", "Möchtest du eine andere Geschichte hören?");
                                int FiftyFifty = new Random().Next(1, 3);

                                if (FiftyFifty == 1)
                                {//ja
                                    Thread.Sleep(1000);
                                    currentReadTime += 1000;
                                    Color("kind", "jaaaaaaaaaaaaaaaa");
                                    return false;
                                }
                                else
                                {//nein
                                    Color("kind", "Mag die Geschichte weiter hören =)");
                                }
                            }
                            else
                            {
                                Color("papa", "Sorry Schatz, wir können jetzt nur diese Geschichte hören");
                            }
                            break;
                        case 3:
                            if (currentReadTime < MAX_READING_TIME)
            ...
            Console.Write(sGeschichte[i]);

            if (sGeschichte[i].ToString().Equals("."))
            {
                Thread.Sleep(PAUSE_AFTER_DOT);
                currentReadTime += PAUSE_AFTER_DOT;
            }
            else
            {
                Thread.Sleep(READING_SPEED);
                currentReadTime += READING_SPEED;
            }
        }
        return true;
    ...
    private static void KidWaitingTime(string v)
    {
        Console.WriteLine("****");
        Console.WriteLine(v);
        Thread.Sleep(9000);
        currentReadTime += 9000;
        Console.WriteLine("****");
    }

    private static bool doesKidWantSmth()
    {
        int iGeneralRandom = new Random().Next(1, 101);

        if (iGeneralRandom <= kidWantsSmthProbability)
        {
            kidWantsSmthProbability = 10;
            return true;
        }
        kidWantsSmthProbability += new Random().Next(5, 11);
        return false;
    }

    public static void Color(string person, string text)
    {
        if (person == "papa")
        {
    ...
    {
        Color("kind", "Papa kannst du eine Geschichte vorlesen?");

        bool GeschichteZuEnde = false;
        do
        {
            Color("papa", "Welche Geschichte möchteste du hören?");

            Color("kind", "Ich hätte gerne...");
            ...
            do
            {
                ...
                if (antwort.Equals("Rotkäppchen"))
                {
                    Console.ForegroundColor = ConsoleColor.Blue;
                    GeschichteZuEnde = vorlesen(sRotkaeppchen);
                    break;
                }
                else if (antwort.Equals("Hänsel und Gretl"))
                {
                    Console.ForegroundColor = ConsoleColor.Blue;
                    GeschichteZuEnde = vorlesen(sHaenselUndGraetel);
                    break;
                }
                else if (antwort.Equals("Aschenputtel"))
                {
                    Console.ForegroundColor = ConsoleColor.Blue;
                    GeschichteZuEnde = vorlesen(sAschenputtel);
                    break;
                }
                Console.WriteLine("Mein Schatz, diese Geschichte kenne ich nicht.");
                antwort = Console.ReadLine();

            }
            while (true);

        } while (!GeschichteZuEnde); // Kid want another Story, kid cannot sleep (je nach wahrscheinlichkeeit), time = 3min, 
    }
