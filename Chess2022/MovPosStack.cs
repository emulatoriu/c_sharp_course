...
namespace SrcChess2 {
    ...
    public class MovePosStack : IXmlSerializable {
        ...
        /// Move list
        /// </returns>
        public MovePosStack Clone() => new MovePosStack(this);
        ...
        public void ReadXml(XmlReader reader) {
            ...
            if (reader.MoveToContent() != XmlNodeType.Element || reader.LocalName != "MoveList") {
                ...
                m_posInList = Int32.Parse(reader.GetAttribute("PositionInList") ?? "-1");
                if (isEmpty) {
                    reader.Read();
                } else {
                    if (reader.ReadToDescendant("Move")) {
                        while (reader.IsStartElement()) {
                            move = new Move {
                                ...
                                StartPos        = (byte)Int32.Parse(reader.GetAttribute("StartingPosition") ?? "0", CultureInfo.InvariantCulture),
                                EndPos          = (byte)Int32.Parse(reader.GetAttribute("EndingPosition") ?? "0",   CultureInfo.InvariantCulture),
