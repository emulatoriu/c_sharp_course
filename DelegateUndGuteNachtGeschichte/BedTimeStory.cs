class BedTimeStory
{
    ...
    private delegate bool thingsTheBabyWillDo();

    private const int START_PROPABILITY = 20;
    private const int CHARACTER_PROP_INCR = 200;
...
kein Blut im Schuck: der Schuck ist nicht zu klein, die rechte Braut, die führt er heim”, gurren die Tauben.Zur Hochzeit wollen sich die Stiefschwestern einschleimen.Doch die Tauben picken beiden jeweils ein Auge aus.Für ihr Bosheit und ihre Falschheit sollen sie ihr Leben lang gestraft sein.";
    ...
    private static bool goToToilet()
    {
    ...
    private static bool goAndDrink()
    {
    ...
    private static bool canNotSleep()
    {
        Thread.Sleep(2000);
    ...
    private static bool anotherStory()
    {
        Thread.Sleep(2000);
    ...
    private static bool startTellingAStory()
    {
        ...
        Console.WriteLine("So mein Schatz, welche Geschichte möchtest du heute lesen? Aschenputtel, Rotkäppchen oder Hänsel und Grätel");
        ...
        do
        {
            foreach (var storyToRead in listOfBetTimeStories)
            {
                if (storyToRead(out story).Equals(chosenStory))
                {
                    bFound = true;
                    break;
                }
        ...
        int probability = START_PROPABILITY;
        int valueToCompareProbability = new Random().Next(1, 101);

        // Read the story, but take care, the longer the story takes, the more the baby wants to
        // break because of a reason
        // so every 100th character there might happen smth.
        for (int i = 0; i < story.Length; i++)
        {
            Console.Write(story.ElementAt(i));

            if (story.ElementAt(i).ToString().Equals("."))
            {
                Thread.Sleep(2000);
            }
            else
            ...
            //    int indexWhatHappens = new Random().Next(0, 4);
            //    Console.WriteLine();
            //    Console.WriteLine(whatBabyCanWant[indexWhatHappens]);
            //    if(!babyDoing[indexWhatHappens]()) 
            //    {//ok baby wants another story
            //        return false;
            //    }
            //    Console.WriteLine();
            //    probability = START_PROPABILITY;
            //}

            if (i % CHARACTER_PROP_INCR == 0 && i>0)
            {
                ...
                Console.WriteLine();
                valueToCompareProbability = new Random().Next(1, 101);
