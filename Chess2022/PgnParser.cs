...
namespace SrcChess2 {
    ...
    public class PgnParserException : System.Exception {
        ...
        public string?  CodeInError { get; private set; }
        ...
        private void CnvSANMoveToPosMove(PgnGame pgnGame, ChessBoard.PlayerColor playerColor, string moveText, out short pos, ref int truncatedCount, ref MoveExt move) {
            ...
            if (index != -1) {
                if (pureMove.Length > index + 1) {
                    ...
                    if (pos != -1) {
                        pureMove = pureMove.Substring(0, index);
                    }
                } else {
                    pos = -1;
                    truncatedCount++;
            ...
            if (pos == 0) {
                if (pureMove == "O-O") {
                    pos = FindCastling(pgnGame, playerColor, true /*bShortCastling*/, ref truncatedCount, moveText, ref move);
                } else if (pureMove == "O-O-O") {
                    pos = FindCastling(pgnGame, playerColor, false /*bShortCastling*/, ref truncatedCount, moveText, ref move);
                } else {
                    int ofs = 1;
                    switch (pureMove[0]) {
                    case 'K':   // King
                    ...
                    DecodeMove(pgnGame, pureMove.Substring(ofs), out int startCol, out int startRow, out int endPos);
                ...
                if (rows.Length != 8) {
                    ...
                    foreach (string row in rows) {
                        ...
                        foreach (char chr in row) {
                            ...
                            default:
                                if (chr >= '1' && chr <= '8') {
                                     blankCount = Int32.Parse(chr.ToString());
                    ...
                    if (retVal) {
                        ...
                        if (cmd != "-") {
                            for (int i = 0; i < cmd.Length; i++) {
                                switch(cmd[i]) {
                                case 'K':
                                    boardStateMask |= ChessBoard.BoardStateMask.WRCastling;
                                    break;
                                case 'Q':
                                    boardStateMask |= ChessBoard.BoardStateMask.WLCastling;
                                    break;
                                case 'k':
                                    boardStateMask |= ChessBoard.BoardStateMask.BRCastling;
                                    break;
                                case 'q':
                                    boardStateMask |= ChessBoard.BoardStateMask.BLCastling;
                                    break;
                                }
                            }
                        }
